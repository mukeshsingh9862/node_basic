...
export const getAllBySort = async (model: Model<any>, query: any, pageNumber: number = 1, pageSize: number = 20, project: any = null, includeSkip: boolean = true,sort:any) => {
    ...
    const totalItems = await model.countDocuments(query); //find(query).count();
...
export const getAllWithoutCount = async (model: Model<any>, query: any, project: any = null) => {
    const items = await model.find(query, project, {lean: { virtuals: true }})
    return {items};
}

// insert or update
export const upsert = async(model: Model<any>, data: any, id?: mongoose.Types.ObjectId | string) => {
    let dataRes = null;
    if(id) {
        // update
        delete data.id;
...
export const oneDaySub = (date: string) => {
    return moment(date).subtract(1, 'day').format('YYYY-MM-DD')
}
export const threeDaySub = (date: string) => {
    return moment(date).subtract(3, 'day').format('YYYY-MM-DD')
}
